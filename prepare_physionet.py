'''
The script is adapted from https://github.com/akaraspt/deepsleepnet 2017
Add some options:
- take all the meaningful channels
- subsampling to provide a lighter dataset
Alex Allauzen, 2024
'''
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="take one sample every n samples, divide the size by n")
    args = parser.parse_args()

    # Input data dir
    if not os.path.exists(args.data_dir):
        raise Exception("data dir does not exist: "+args.data_dir)
    ...
    allch = ["EEG Fpz-Cz", "EEG Pz-Oz", "EOG horizontal", "EMG submental"]
    if select_ch == "all":
        print("Select all the channels !!!")
        select_ch = allch
    ...
    for i in range(len(psg_fnames)):
        ...
        f.close()
        raw_start_dt = datetime.strptime(h_raw['date_time'],
                                         "%Y-%m-%d %H:%M:%S")
        ...
        f.close()
        ann_start_dt = datetime.strptime(h_ann['date_time'],
                                         "%Y-%m-%d %H:%M:%S")
        ...
        if len(raw_ch) % (EPOCH_SEC_SIZE * sampling_rate) != 0:
            print(raw_ch, EPOCH_SEC_SIZE, sampling_rate, n_epochs)
